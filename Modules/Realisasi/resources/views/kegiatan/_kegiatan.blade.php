        ...
        @foreach ($data->kegiatan as $item)
            ...
            <tr>
                ...
                @if ($item->indikator_keg)
                    ...
                    <td>{{ $item->satuan_keg }}</td>
                    <td>{{ number_format($item->volume_keg) }}</td>
                    <td>
                        <input required type="text" name="volume_realisasi[]" value=""
                            class="form-control mb-2 mr-sm-2 nominal" placeholder="Volume">
                    </td>
                @else
                    <input type="hidden" name="volume_realisasi[]" value="" class="form-control mb-2 mr-sm-2 nominal"
                        placeholder="Volume">
