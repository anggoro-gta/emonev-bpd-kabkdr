    ...
    <section class="section">
        <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
                <a href="{{ route('realisasi.sub_kegiatan.create') }}" class="btn btn-success" title="Tambah"><i
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($item->status_posting==0)
                                                <a href="{{ route('realisasi.sub_kegiatan.edit',$item->id) }}"
                                                    class="btn btn-primary" title="Edit"><i
                                                        class="fas fa-pencil"></i></a>

                                                <a data-url="{{ route('realisasi.sub_kegiatan.tooglePosting',$item->id) }}" data-text="Anda Yakin Posting" data-status_posting="1"
                                                    title="Posting" class="btn btn-success togglePosting">
                                                    <i class="fas fa-check"></i>
                                                </a>
                                            @else

                                            <a href="{{ route('realisasi.sub_kegiatan.show',$item->id) }}"
                                                class="btn btn-primary" title="Edit"><i
                                                    class="fas fa-eye"></i></a>
                                                @if(auth()->user()->hasRole('Admin'))
                                                    <a data-url="{{ route('realisasi.sub_kegiatan.tooglePosting',$item->id) }}" data-text="Anda Yakin Batal Posting" data-status_posting="0"
